...
def compare_classification(label_name, model, device, user):
        ...
        if model == "SVM":
                ...
                pickling(model + '_' + str(user) + "_Vanilla_hmog.pkl", clf)
...
file_ptr = open("Vanilla_Results_hmog.out", "w")
...
for model in models:
    ...
    for val in range(1, 100):
        ...
        pickling("X"+str(val)+"_hmog.pkl", X_test)
        pickling("Y"+str(val)+"_hmog.pkl", y_test)
        ...
        pickling("scaler_Vanilla_phone_hmog.pkl", scaler)
        pickling("transformer_Vanilla_phone_hmog.pkl",transformer)
