...
scaler = unpickling("scaler_GAN_Phone_selectK_hmog.pkl")
transformer = unpickling("transformer_GAN_Phone_selectK_hmog.pkl")
gender_list = parse_demographics()

file_ptr = open("GAN_Attack_SVM_random_hmog.out", "w")
...
for model in models:
    ...
    for i in range(1,37):
        if i == 20:
            continue
        #acc, far, frr, hter = attack3("hmog_features.pkl",str(model)+"_"+str(i)+"GAN_Phone_GAN.pkl")
        #acc, far, frr, hter = attack3("serwadda_features.pkl",str(model)+"_"+str(i)+"GAN_Phone_GAN.pkl")
        acc, far, frr, hter = attack3("random_attack_data.pkl",str(model)+"_"+str(i)+"GAN_Phone_GAN_hmog.pkl")
        #acc, far, frr, hter = attack3("hmog_features.pkl",str(model)+"_"+str(i)+"GAN_Phone_GAN_hmog.pkl")
        #acc, far, frr, hter = attack2(X, u, i,str(model)+"_"+str(i)+"GAN_Phone_GAN.pkl")
        #acc, far, frr, hter = attack2(X, u, i,"RForest_"+str(i)+"GAN_Phone_selectK.pkl")
        #acc, far, frr, hter = attack("pickle_files/pop_data_10000.pkl","MLP_"+str(i)+"Phone_Vanilla_selectK.pkl")
        #acc, far, frr, hter = attack("pickle_files/pop_data_10000.pkl",str(model)+"_"+str(i)+"GAN_Phone_GAN.pkl")
        final_acc.append(acc)
        final_far.append(far)
        final_frr.append(frr)
        final_hter.append(hter)

        file_ptr.write("{},{},{},{}\n".format(acc,far,frr,hter))
