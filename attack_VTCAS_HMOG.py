...
u = unpickling("hmog_features.pkl")
u = np.array(u)
X = unpickling("hmog_features.pkl")
X = np.array(X)

scaler = unpickling("scaler_Vanilla_phone_hmog.pkl")
transformer = unpickling("transformer_Vanilla_phone_hmog.pkl")
gender_list = parse_demographics()

file_ptr = open("Vanilla_Attack_random_hmog.out", "w")
...
for model in models:
    ...
    final_hter_f = []
    for i in range(1,37):
        # Use any attack attack data instead of random_attack_data.pkl
        acc, far, frr, hter = attack3("random_attack_data.pkl",str(model)+"_"+str(i)+"Vanilla_Phone_Vanilla_hmog.pkl")
