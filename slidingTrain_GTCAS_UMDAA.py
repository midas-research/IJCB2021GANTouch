...
def compare_classification(label_name, model, device, user):
        ...
        if model == "SVM":
                ...
                pickling(model + '_' + str(user) + "_GAN_umdaa.pkl", clf)
...
def get_data(legitimate, size = 2):
  data = unpickling("umdaa_features.pkl")
  p = list(range(1, 37))
...
file_ptr = open("GAN_Results_umdaa_exp1.out", "w")
...
for model in models:
    ...
    for val in range(1, 37):
        X, y = get_data(val, 3)
        X_matrix, y_vector = ADASYN(random_state=0, sampling_strategy='minority').fit_resample(X, y)
        X_train, X_test, y_train, y_test = train_test_split(
                        X_matrix, y_vector, test_size=0.4, stratify = y_vector, random_state=0)

        pickling("X_Legit_NonGAN_umdaa.pkl", X_train[[np.where(y_train==1)]])
        pickling("X_Adver_NonGAN_umdaa.pkl", X_train[[np.where(y_train==0)]])

        X1 = np.concatenate((generate_samples(X_train[np.where(y_train==1)], 2000),X_train[np.where(y_train==1)]), axis=0)

        X0 = np.concatenate((generate_samples(X_train[np.where(y_train==0)], 2000),X_train[np.where(y_train==0)]), axis=0)

        pickling("X_Legit_GAN_umdaa.pkl", X1)
        pickling("X_Adver_GAN_umdaa.pkl", X0)

        X1 = unpickling("X_Legit_GAN_umdaa.pkl")
        X0 = unpickling("X_Adver_GAN_umdaa.pkl")
        ...
        pickling("scaler_GAN_Phone_selectK_umdaa.pkl", scaler)
        pickling("transformer_GAN_Phone_selectK_umdaa.pkl", transformer)
        res = []
