...
u = np.array(u)
X = unpickling("umdaa_features.pkl")
X = np.array(X)

scaler = unpickling("scaler_Vanilla_phone_umdaa.pkl")
transformer = unpickling("transformer_Vanilla_phone_umdaa.pkl")
gender_list = parse_demographics()

file_ptr = open("Vanilla_Attack_hmog_umdaa.out", "w")
...
for model in models:
    ...
    final_hter_f = []
    for i in range(1,37):
        # Replace hmog_features.pkl with any attack data.
        acc, far, frr, hter = attack3("hmog_features.pkl",str(model)+"_"+str(i)+"Vanilla_Phone_Vanilla_umdaa.pkl")
