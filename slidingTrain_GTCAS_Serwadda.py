...
def compare_classification(label_name, model, device, user):
        ...
        if model == "SVM":
                ...
                pickling(model + '_' + str(user) + "_GAN_serwadda.pkl", clf)
...
file_ptr = open("GAN_Results_serwadda.out", "w")
...
for model in models:
    ...
    for val in range(1,137):
        ...
        pickling("X_Legit_NonGAN_serwadda.pkl", X_train[[np.where(y_train==1)]])
        pickling("X_Adver_NonGAN_serwadda.pkl", X_train[[np.where(y_train==0)]])
        ...
        pickling("X_Legit_GAN_serwadda.pkl", X1)
        pickling("X_Adver_GAN_serwadda.pkl", X0)

        X1 = unpickling("X_Legit_GAN_serwadda.pkl")
        X0 = unpickling("X_Adver_GAN_serwadda.pkl")
        ...
        pickling("scaler_GAN_Phone_selectK_serwadda.pkl", scaler)
        pickling("transformer_GAN_Phone_selectK_serwadda.pkl", transformer)
        res = []
