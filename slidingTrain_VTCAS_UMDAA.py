...
def compare_classification(label_name, model, device, user):
        ...
        if model == "SVM":
                ...
                pickling(model + '_' + str(user) + "_Vanilla_umdaa.pkl", clf)
...
file_ptr = open("Vanilla_Results_umdaa.out", "w")
...
for model in models:
    ...
    for val in range(1, 37):
        ...
        pickling("X"+str(val)+"_umdaa.pkl", X_test)
        pickling("Y"+str(val)+"_umdaa.pkl", y_test)
        ...
        pickling("scaler_Vanilla_phone_umdaa.pkl", scaler)
        pickling("transformer_Vanilla_phone_umdaa.pkl",transformer)
