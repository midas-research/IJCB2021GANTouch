...
def compare_classification(label_name, model, device, user):
        ...
        if model == "SVM":
                ...
                pickling(model + '_' + str(user) + "_GAN_hmog.pkl", clf)
...
def get_data(legitimate, size = 2):
  data = unpickling("hmog_features.pkl")
  p = list(range(1, 100))
...
file_ptr = open("GAN_Results_hmog.out", "w")
...
for model in models:
    ...
    for val in range(1,100):
        ...
        pickling("X_Legit_NonGAN_hmog_"+str(val)+".pkl", X_train[[np.where(y_train==1)]])
        pickling("X_Adver_NonGAN_hmog_"+str(val)+".pkl", X_train[[np.where(y_train==0)]])
        ...
        pickling("X_Legit_GAN_hmog_"+str(val)+".pkl", X1)
        pickling("X_Adver_GAN_hmog_"+str(val)+".pkl", X0)

        X1 = unpickling("X_Legit_GAN_hmog_"+str(val)+".pkl")
        X0 = unpickling("X_Adver_GAN_hmog_"+str(val)+".pkl")
        ...
        pickling("scaler_GAN_Phone_selectK_hmog.pkl", scaler)
        pickling("transformer_GAN_Phone_selectK_hmog.pkl", transformer)
        res = []
